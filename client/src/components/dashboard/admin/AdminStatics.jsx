...
const AdminStatics = () => {
    ...
    const crisisCounts = {};
    reports.forEach((r) => {
        const t = r.crisisType || 'unknown';
        crisisCounts[t] = (crisisCounts[t] || 0) + 1;
    });
    const crisisTypeData = Object.entries(crisisCounts).map(([name, value]) => ({
        name: CRISIS_LABELS[name] || name,
        value,
    }));
    ...
    const reportData = {
        daily: dailyData.length > 0 ? dailyData : [{ date: 'No data', count: 0 }],
    };
    const stats = {
    ...
    return (
        <div className="space-y-4 sm:space-y-6" ref={dashboardRef}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
                ...
                <div className="flex flex-wrap gap-2 w-full sm:w-auto mt-2 sm:mt-0"></div>
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
                <div className="bg-white p-3 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                    <div className="text-blue-600 text-sm sm:text-lg font-semibold mb-1">
                        Total Reports
                    </div>
                    <div className="text-xl sm:text-3xl font-bold">{stats.totalReports}</div>
                </div>
                <div className="bg-white p-3 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                    <div className="text-yellow-600 text-sm sm:text-lg font-semibold mb-1">
                        Active Crises
                    </div>
                    <div className="text-xl sm:text-3xl font-bold">{stats.activeReports}</div>
                </div>
                <div className="bg-white p-3 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                    <div className="text-purple-600 text-sm sm:text-lg font-semibold mb-1">
                        Total Alerts
                    </div>
                    <div className="text-xl sm:text-3xl font-bold">{stats.totalAlerts}</div>
                </div>
                <div className="bg-white p-3 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                    <div className="text-red-600 text-sm sm:text-lg font-semibold mb-1">
                        Critical Events
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-3 sm:p-6">
                    ...
                    <div className="mt-3 sm:mt-4 grid grid-cols-2 gap-1 sm:gap-2">
                        {(crisisTypeData.length > 0 ? crisisTypeData : [])
                            .slice(0, 4)
                            .map((item, index) => (
