...
const LoginForm = () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setStatus({
                    loading: false,
                    success: false,
                    error: data.message || t('auth.login.authFailed'),
                });
            }
        } catch (_error) {
            setStatus({
                loading: false,
                success: false,
                error: t('auth.login.connectionError'),
            });
        }
    };
    ...
    const renderSuccessOverlay = () => (
        <div className="absolute inset-0 bg-white bg-opacity-95 flex flex-col items-center justify-center rounded-xl z-10 p-6">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">{t('auth.login.success')}</h3>
            <p className="text-gray-600 mb-6">{t('auth.login.redirecting')}</p>
    ...
    const buttonClassName = `group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition duration-150 ease-in-out ${
        status.loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700 focus:ring-red-500'
    }`;
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg relative">
                ...
                <div className="text-center">
                    ...
                    <h2 className="mt-2 text-3xl font-extrabold text-gray-900">{t('auth.login.title')}</h2>
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div>
                        <button type="submit" className={buttonClassName} disabled={status.loading}>
                            {status.loading ? (
                                <>
                                    <FiLoader className="animate-spin h-5 w-5 me-2" />
