...
class AlertsRepository {
    ...
    async findAll() {
        const query = `
            SELECT id, message, description, date, "time"::text, status, importance, type,
                   location, affected_area as "affectedArea", created_at
    ...
    async update(id, updates) {
        ...
        let index = 1;
        const mapping = {
            message: 'message', description: 'description', status: 'status',
            importance: 'importance', type: 'type', location: 'location',
        ...
        values.push(id);
        const query = `UPDATE alerts SET ${fields.join(', ')}, created_at = CURRENT_TIMESTAMP WHERE id = $${index}`;
